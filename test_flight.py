...
# lift / thrust default to TRUE = tiltrotor, so a CSV predating these columns
# reads as the all-tiltrotor fleet it described.
        ...
        with open(_ROTOR_CSV, newline="") as fh:
            ...
            for row in reader:
                ...
                if powerplant != _S4_POWERPLANT:
                    changed = True
                # lift / thrust — rotor mode, consumed by rotor_mixer.jl via
                # RotorUnit. Always written so rotor_system.jl never has to
                # guess; any non-tiltrotor rotor forces an override entry.
...
# ══════════════════════════════════════════════════════════════════════════════
# Stage 4 — Analysis (test checks, console report)
# Replaces the former subprocess calls to plot_results.py and test_executive.py,
# both of which are now defunct. Plotting lives in plot_test_flight.py — run it
# separately against the resulting CSV. HTML/markdown report files are retired;
# windblade.py's own output covers that ground now.
# ══════════════════════════════════════════════════════════════════════════════
...
def run_analysis(csv_path: Path, card_path: Path, out_dir: Path) -> int:
    ...
    # Console summary — HTML/markdown reports are retired; windblade.py's own
    # output supersedes them, so this is the sole report surface now.
...
def main():
    ...
    p.add_argument("--out",      default=str(ROOT), metavar="DIR",
                   help="Output directory for the CSV (and .db, if --db).")
